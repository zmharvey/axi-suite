// Guided token capture: hidden interactive paste (TTY) or piped stdin (scripts).
// Fixes the fragile manual token-file drop.
...
export const AUTH_HELP = `usage: auth <login|logout>
  login    paste your token — input is hidden; also reads piped stdin for scripts
...
export async function authCommand(args, meta) {
  ...
  const token = process.stdin.isTTY
    ? await promptHidden(`Paste your ${meta.tool} token (input hidden), then press Enter:\n`)
    : await readPiped();
  if (!token) throw new AxiError("No token provided", "VALIDATION_ERROR", meta.hint ? [meta.hint] : []);
